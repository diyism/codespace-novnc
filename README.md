...
deepnote.com 有docker image部署(+init.ipynb) + jupyter notebook + terminal, 有root有socket权限没有cap-add MKNOD权限(/dev/net/tun), 功能非常全面, 但是有机器人严格监测vnc,porxy等进程而自动suspend user, 所以无法部署这个项目, 固定listen udp 443对不同目标出去的端口也不同, 所以也不能部署naive_caddy, 磁盘5GB
...
render.com,noteable.io   即使listen在固定的443端口, 每个不同目标出去的端口都不同, 无法使用naive_caddy
